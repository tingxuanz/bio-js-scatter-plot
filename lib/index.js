...
module.exports = biojsvisrohartmsctest = function(init_options)
{
   ...
   this.setup_line = function(graph){
    d3.csv("../data/ds_id_2000_scatter_pdgfd.tsv", function(error, data) {
        ...
        scatter_line = d3.svg.line()
            .x(function(d) { return scaleX(calculate_x_value_of_sample_types);})
            .y(function(d) { return scaleY(d.Expression_Value);});

         svg.append("path")
            .attr("class", "line")
            .attr("d", scatter_line(data));
        graph.svg = svg;
        return graph;   
//(d3.extent(options.data, (function(d) { return scaleY(d.Expression_Value);})));
});
  ...
  // draw legend colored rectangles
        legend.append("rect")
          .attr("x", (graph.full_width - options.margin.left - 95))
          .attr("width", 18)
          .attr("height", 18)
          .style("fill", color);
  ...
  // draw legend text
        legend.append("text")
          .attr("x", (graph.full_width - options.margin.left - 100))
          .attr("y", 9)
          .attr("dy", ".35em")
          .style("text-anchor", "end")
