    ...
    */
    ...
    this.setup_x_axis_using_sample_types = function (graph){
        ...
        svg.selectAll(".sample_type_text")  // text for the xaxes - remember they are on a slant 
           ...
           .text(
            ...
            .on('mouseover', function (d) {
                ...
                $(".sample-type-" + d.sample_type.replace(/\ |(|)/g,'')).hover();
                var im = document.getElementsByClassName("sample-type-" + d.sample_type.replace(/\ |(|)/g,''));
               // im.style.fill = "red";
               // im.style.stroke = "10px";
        ...
        .on('mouseout', function (d) {
            ...
            var im = document.getElementsByClassName("sample-type-" + d.sample_type.replace(/\ |(|)/g,''));
            for (i = 0; i < im.length; i++) {
                d3.select(im[i]).attr("r", options.circle_radius).style("opacity", 1);
            }
        });
        ...
        */
        ...
        svg.selectAll(".dot") // class of .dot
            ...
            .attr("class", function(d) {
                    return "sample-type-" +  d.Sample_Type.replace(/\ |(|)/g,''); }) 
    ...
    var div = document.createElement("div");
        ...
        svg.selectAll(".bar")
            ...
            .attr("width", function(d, i) {
                ...
                if(once == 0) {
                    once = 1;
                    return scaleX(d.end_sample_id) - scaleX(d.start_sample_id) + (3/2)*padding;
                } if (sample_id_count == 0) {//(scaleX(d.end_sample_id) >= options.width - (2 * padding)) {
            ...
            .on("mouseover", function(d) {     
                div.innerHTML =  "Sample Type: " +  d.sample_type + "<br/>";
                div.transition()
                .duration(500)
                .style("opacity", 1);
                })
                // tip.style('left', Math.max(0, d3.event.pageX - 150) + "px");
             //tip.style('top', (d3.event.pageY + 20) + "px");
            //    tip.style('color', "red");
            
            .on("mousemove", function(d) {
                document.getElementById("tooltip-div").innerHTML =  "Sample Type: " +  d.sample_type + "<br/>";
                //div.innerHTML(d3.event.pageX + ", " + d3.event.pageY);
                div.style('left', Math.max(0, d3.event.pageX - 150) + "px");
       ...
       //       document.body.appendChild(div);      
     /*          var barPosition = this.position();
                        $span.
                            css("top",(d.pageY - barPosition.top + 100) + "px").
                            css("left",(d.pageX - barPosition.left + 100) + "px");
                    });*/
           // tip
      ...
      //sourced from: http://codepen.io/recursiev/pen/zpJxs
...
/*    var position = document.rootElement.createSVGPoint();
    ...
    var mousey = correctPosition.y; 
             mousePosition = d3.mouse(SVGmouseTip.node().parentElement);//.on('mouseover', tip.show) //displays the sample type above the graph
            SVGmouseTip
