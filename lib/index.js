    ...
    this.setup_x_axis_using_sample_types = function (graph){
        ...
        svg.selectAll(".sample_type_text")  // text for the xaxes - remember they are on a slant 
           ...
           .text(
            ...
            .on("mouseover", highlight_dots)
            .on("mouseout", function(d) {
                d3.select(this)
                .attr("fill", "black")});
    
        function highlight_dots(d) {
            d3.selectAll($("#" + d.sample_type))
                .style("fill", "red");
        }
    ...
    this.setup_verical_select = function(graph) {
        ...
        svg.selectAll(".bar")
            ...
            .attr("x", function(d){
                if(first == 0) {
                    first = 1;
                    return scaleX(d.start_sample_id) - 10;
                }
                return temp = scaleX(d.start_sample_id) - 5;
               })
            .attr("width", function(d, i) { 
                if(once == 0) {
                    once = 1;
                    return scaleX(d.end_sample_id) + 5;
                } else {
                    return scaleX(d.end_sample_id) - scaleX(d.start_sample_id) + 10;
                }
            })
            .attr("y", 0)
