...
module.exports = biojsvisrohartmsctest = function(init_options)
{
    ...
    this.default_options = function(){
        ...
        var options = {
            target: "#graph",
            unique_id: "Sample_ID",
            margin:{top: 80, right: 0, bottom: 30, left: 40},
            height: 1500,
            width: 1060,
...
** http://bl.ocks.org/dougdowson/8a43c7a7e5407e47afed*/         
      ...
      svg.append("g")
            .attr("class", "grid")  
            .call(yAxis
                .tickSize(-page_options.width, 0, 0)
               // .tickFormat(" ")
            )
       //   .attr("class", "y_axis")
       //   .call(yAxis) // this is actually implementing the yAxis as an axis itself
        ...
        for (temp_count in nested_values){
            ...
            for (count_row in values){
                ...
                if($.inArray(sample_id, sample_id_list) == -1) {
                  //  sample_type_list.push(sample_type);//if the sample isn't in the types array add it
                 //   type_count += 1; //add 1 to the tpye count so we know the number of different types
            ...
            temp['start_sample_id'] = values[0].Sample_ID;//Replicate_Group_ID;
...
/*Was stoping the values being displayed on the y axis        
                svg.selectAll("g")
            .attr("class","tick")
            .remove();
*/
    ...
    this.setup_error_bars = function (graph){
        ...
        */
        ...
        svg.selectAll(".max").data(options.data).enter()
            ...
            .attr("y1", 
                function(d){
                    temp = scaleY(d.upr);//d.Expression_Value);// + d.Standard_Deviation);//(d.upr);
                    return temp;
                }
            )
            .attr("y2", 
                function(d){
                    temp = scaleY(d.upr);//d.Expression_Value);//d.upr);//(d.Expression_Value + d.Standard_Deviation);//(d.upr);
                    return temp;
                }
            )
        ...
        svg.selectAll(".min").data(options.data).enter()
            ...
            .attr("y1", 
                function(d){
                    temp = scaleY(d.lwr);//d.Expression_Value - d.Standard_Deviation);//(d.lwr);
                    return temp;
                }
            )
            .attr("y2", 
        ...
        svg.selectAll(".vertical").data(options.data).enter()
            ...
            .attr("y1", 
                function(d){
             //       temp = scaleY(d.Expression_Value + d.Standard_Deviation);//(d.upr);
                    temp = scaleY(d.upr);//d.Expression_Value + d.Standard_Deviation);//(d.upr);
                    return temp;
                }
            )
...
//     This is to setup multiple horizontal lines with a label
    ...
    this.setup_horizontal_lines = function(graph){
      /*   svg = graph.svg;
         horizontal_lines = graph.horizontal_lines;
       // sample_id_list = graph.sample_id_list;
        page_options = graph.page_options;
       // calculate_x_value_of_vertical_lines = this.calculate_x_value_of_vertical_lines;

        svg.selectAll(".line").data(horizontal_lines).enter()
            .append("line")
            .attr("class", "separator")
            .attr("x1",
                function(d){
            ...
            .attr("y2",
                function(d){
                    // this is to keep it within the graph
                    temp = scaleY(5);// page_options.height;
                    return temp;
                }
            )
            .attr("shape-rendering","crispEdges")
    ...
    this.setup_rohart_background_text = function(graph){
        ...
        */
        ...
        svg.append("text")
            ...
            .attr("y", scaleY(options.dt) - 10)//horizontal_lines['upr'])-margin_y_value_msc)
            .text("DT")
            .attr("text-anchor", "middle")  
            .style("font-family", "Arial")  
            .style("font-size", font_size)  
        ...
        svg.append("text")
            .attr("x", (page_options.width / 2))             
            .attr("y", scaleY(options.med) - 10)//options.horizontal_lines['lwr'])+margin_y_value_msc)
            .text("MED")
            .attr("text-anchor", "middle")  
            .style("font-family", "Arial")  
            .style("font-size", font_size)  
