        ...
        svg.selectAll(".bar")
            ...
            .attr("x", function(d){
                if(first == 0) {
                    ...
                    return scaleX(d.start_sample_id) - (padding * (2/3));
                } else {
                    return temp = scaleX(d.start_sample_id) - padding/2;
               } 
            })
            .attr("width", function(d, i) { 
                if(once == 0) {
                    once = 1;
                    return scaleX(d.end_sample_id) + padding;
                } if (scaleX(d.end_sample_id) >= options.width - (2 * padding)) {
                    //if it is the last sample type need to account for padding of the graph
                    //append 15 to the bar
                    return scaleX(d.end_sample_id) - scaleX(d.start_sample_id) + (1.5 * padding);
                } else {
