   ...
   */ 
    ...
    */
    ...
    this.setup_x_axis_using_sample_types = function (graph){
        ...
        svg.selectAll(".sample_type_text")  // text for the xaxes - remember they are on a slant 
            ...
            .on('mouseover', function (d) {
                ...
                var sample_type_group = document.getElementsByClassName("sample-type-" + options.sample_types[d.sample_type]);//+d.sample_type_id);//sample_type_count);//d.sample_type.replace(/\ |(|)/g,''));
    ...
    this.setup_error_bars = function (graph){
        ...
        stroke_width = 0;
        if(options.graph_size == "small") {
            stroke_width = options.stroke_width.small;
        } else {
            stroke_width = options.stroke_width.large;
        }
        ...
        for (var j = 0; j < (2*columnCount); j++) {
            ...
            if(probe_number + 1 <= options.probe_count){
                ...
                for (i = 0; i < probe_group.length; i++) {
                    if(probe_group[i].style.color !== "white") {
                        d3.select(probe_group[i]).style("opacity", 0);
                        probe_group[i].style.color = "white";
                    } else {
                        probe_group[i].style.color = "blue";
