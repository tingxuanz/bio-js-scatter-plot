 ...
 */
    ...
    var btn = document.createElement('input');
        ...
        btn.style.font = "20px sans-serif";
     //   btn.style.left = page_options.width + page_options.margin.right;
   //     btn.style.top = page_options.margin.top - 20;
...
var div1 = document.createElement("div");
         ...
         //   div.style.height = "100px";
          ...
          //  div.style.background = "red";
            ...
            div1.style.left = page_options.width + page_options.margin.right;
            div1.style.top = page_options.margin.top - 50;
 ...
 /*   for (var i = 0; probe_number < options.probe_count; i++) {
        row = table.insertRow(-1);
        probe_number += i;
        for (var j = 0; j < (2*columnCount) + 1; j++) {
            var cell = row.insertCell(-1);
            if(j%2 == 1){
                btn = document.createElement('input');
                btn.type = "button";
                btn.className = "btn";
                btn.style.background = options.probes[i][1];
                cell.appendChild(btn);
            } else {
                probe_number += j;
                cell.innerHTML = options.probes[i][0];
            }
        }
     }
*/
