...
module.exports = biojsvisscatterplot = function(init_options)
{
    ...
    //Setup the options for a small graph
    this.setup_size_options = function(graph){
        options = graph.options;
        if(options.graph_size == "small") {
            size_options.title_text = options.title_text_size.small;
            size_options.horizontal_grid_lines = 0;
            size_options.text_size = options.text_size.small;
            size_options.x_axis_title = options.x_middle_title.small;
            size_options.increment = (options.increment/2 | 0);
            size_options.padding = 20;
        } else {
            size_options.title_text = options.title_text_size.large;
            size_options.text_size = options.text_size.large;
            size_options.horizontal_grid_lines = options.horizontal_grid_lines;
            size_options.x_axis_title = options.x_middle_title.large;
            size_options.increment = options.increment;
            size_options.padding = 50;
        }

        graph.size_options = size_options;
        return graph;
   ...
   */ 
    ...
    this.setup_scatter_line = function(graph){
         //don't setup the scatter_line if the graph is small
       /* if(options.graph_size == "small") {
            return graph;
        }*/ 
    ...
    */
    ...
    this.setup_x_axis_using_sample_types = function (graph){
        ...
        radius = 0;
        if(options.graph_size == "small") {
            radius =  options.circle_radius.small;
        } else {
            radius = options.circle_radius.large;
        }
        ...
        svg.selectAll(".sample_type_text")  // text for the xaxes - remember they are on a slant 
            ...
            .on('mouseover', function (d) {
                sample_type_count++;
                console.log(options.sample_types[d.sample_type]);
                $(".sample-type-" + options.sample_types[d.sample_type]).hover();
                var im = document.getElementsByClassName("sample-type-" + options.sample_types[d.sample_type]);//+d.sample_type_id);//sample_type_count);//d.sample_type.replace(/\ |(|)/g,''));
                for (i = 0; i < im.length; i++) {
                    d3.select(im[i]).attr("r", options.hover_circle_radius).style("opacity", 0.5);
            }
        })
        .on('mouseout', function (d) {
            console.log(d.sample_type.split(" "));
            $(".sample-type-" +  options.sample_types[d.sample_type]);//d.sample_type_id).hover();//d.sample_type.replace(/\ |(|)/g,'')).hover();
            var im = document.getElementsByClassName("sample-type-" + options.sample_types[d.sample_type]);//d.sample_type_id);//+ sample_type_count);//d.sample_type.replace(/\ |(|)/g,''));
            for (i = 0; i < im.length; i++) {
                d3.select(im[i]).attr("r", radius).style("opacity", 1);
            }
        });
    ...
    var div_legend_button = document.createElement("div");
            ...
            div_legend_button.style.fontSize = size_options.title_text;
            div_legend_button.style.left = page_options.width + page_options.margin.right + 10;
            div_legend_button.style.top = page_options.margin.top - page_options.margin.top - size_options.padding;
    ...
    for (var i = 0; i < options.probe_count + 2; i++) {
        ...
        for (var j = 0; j < (2*columnCount); j++) {
            ...
            if(probe_number + 1 <= options.probe_count){
                ...
                var probe = (this.id);
                //var probe = options.probes[0][0];
                var im = document.getElementsByClassName("line-probe-" + probe.replace(/\ |(|)/g,''));
                for (i = 0; i < im.length; i++) {
                    if(im[i].style.color !== "white") {
                        d3.select(im[i]).style("opacity", 0);
                        im[i].style.color = "white";
                    } else {
                        im[i].style.color = "blue";
                         d3.select(im[i]).style("opacity", 1);
                    }
                }
            };
    ...
    //toggle the scatterLine
    toggle_scatter = function(probe_number) {
              //   $(".probe-" + options.[probe_number][0].split(" "));
                var im = document.getElementsByClassName(".probe-" + options.probes[probe_number][0].replace(/\ |(|)/g,''));
                for (i = 0; i < im.length; i++) {
                    if(im[i].style.color !== "white") {
                        d3.select(im[i]).style("opacity", 0.1);
                        im[i].style.color = "white";
                    } else {
                        im[i].style.color = "blue";
                        d3.select(im[i]).style("opacity", 0.1);
    ...
    this.setup_horizontal_lines = function(graph){
        ...
        font_size = size_options.text_size;
        margin_y_value = 20;
    ...
    this.setup_hover_bars = function(graph) {
        ...
        svg.selectAll(".bar")
            ...
            .on("mouseout", function(d) {
                tooltip_sample.style("opacity", 0);
            });
