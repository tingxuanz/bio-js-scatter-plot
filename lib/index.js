...
module.exports = biojsvisrohartmsctest = function(init_options)
{
   ...
   //http://bl.ocks.org/d3noob/e99a762017060ce81c76 helpful for nesting the probes
    this.setup_scatter_line = function(graph){
        ...
        //Gets the colours for the probes
        colour = options.colour; 
        colour_count = 0;//d3.scale.category20();
        //nests through for each probe (doesn't take order into account)
        //order needs to be done separately
    ...
    this.setup_y_axis = function(graph){
       ...
       /* grid_lines = function(d){
            if(options.y_lines == "yes"){
                 grid_lines = -page_options.width;
            } else {
                grid_lines = 0;
            }
        } */
   ...
   //     graph.horizontal_lines = horizontal_lines;
    ...
    this.setup_x_axis = function (graph){
        ...
        var scaleX = d3.scale.ordinal()
            .rangeBands([10, page_options.width]); // note that 0.4 was chosen by iterative fiddling
    ...
    this.setup_x_axis_using_sample_types = function (graph){
        ...
        svg.selectAll(".sample_type_text")  // text for the xaxes - remember they are on a slant 
           ...
           .text(
            ...
            .on("mouseover", highlight_dots)function(d) {
                d3.select(this)
                .attr("fill", "orange")})
        ...
        function highlight_dots(d) {
            var dot = d3.select(this)
                .style("fill", "red");
            var line = d3.select(".sample-type-" +  d.sample_type)
                    .style("fill", "blue");
...
*/
    ...
    //    var cValue = function(d, i) { return d.Probe;},
   //     colour = options.colour;
...
//function(d) { return (cValue(d, i));})
   ...
   function highlight_dots(d) {
            tooltip.show;
            var line = d3.select(".dot sample-type-" +  d.Sample_Type)
                    .style("fill", "red");
 ...
 */
    ...
    var legend_btn = document.createElement('input');
        ...
        legend_btn.style.font = "20px sans-serif";
        legend_btn.style.backgroundColor = "white";
        legend_btn.style.color = "#FF3300";
...
var div_legend_button = document.createElement("div");
    ...
    probe_number = 0;
    colour_count = 0;
  for (var i = 0; probe_number < options.probe_count + 2; i++) {
...
/*    svg.selectAll(".bar")
      .data(options.data)
        .enter().append("rect")
        .attr("class", "bar")
        .attr("x", function(d) { return scaleX(d[options.x_column]); })
        .attr("width", scaleX(10))
        .attr("y", function(d) { return scaleY(1) })
        .attr("height", function(d) { return scaleY(5); })
...
//     This is to setup multiple horizontal lines with a label
    ...
    this.setup_verical_select = function(graph) {
        svg = graph.svg;
        options = graph.options;
        tip = options.tip;
        svg.call(tip);
        ...
        svg.selectAll(".bar")
            ...
            .attr("class", "bar")
            .attr("x", function(d){
                temp = (scaleX(d.start_sample_id));
                if(temp < page_options.width && temp !== 0) {
                    return (scaleX(d.start_sample_id)) - 5;
                } else { return page_options.width;}}) /*(once == 0) {
                    return page_options.width - 190;
                }
                })
                /*
                var start_temp= scaleX(d.start_sample_id);
            ...
            .attr("width", function(d) { 
                temp = (scaleX(d.end_sample_id));
                if(temp  < page_options.width - 50 && temp !== 0) {
                    return (scaleX(d.end_sample_id) + 50);
                } else if (once == 0) {
                    once = 0;
                    return scaleX(d.start_sample_id) + 50;
                }
    ...
    this.setup_watermark = function(graph){
        ...
        svg.append("image")
            ...
            .attr("y", -page_options.width - 100)//-(graph.full_width - options.margin.left)) // just out of the graphs edge
            .attr("transform", "rotate(+90)")
            .attr("width", 200)
            .attr("height", 100);
    ...
    **Setting up the y axis label (d2 logarithmic) */
    ...
    this.setup_graph = function(graph){
      ...
      //  graph = this.setup_y_axis(graph);
        ...
        graph = this.setup_verical_select(graph);
        graph = this.setup_x_axis(graph);
        graph = this.setup_y_axis(graph);
