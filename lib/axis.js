    ...
    */
    setup_data_for_x_axis = function(graph){
        ...
        for (var p in nested_values) {
            ...
            probe = row.key;
            values = row.values;
            if (options.multi_group != 1) { //There will be a second key which is the disease state
                tempX_outer = {};
                for (var st in values) {
                    var srow = values[st];
                    disease_state = srow.key;
                    dvals = srow.values;
                    tempX = {};
                    for (var ds in dvals) {
                        inner_tempX = {};
                        ...
                        cur_name = remove_chars(probe + "-" + sample_type + "-" + disease_state);
                        ...
                        tempX = {};
                        tempX['label1'] = probe;
                        tempX['label2'] = disease_state;
                        tempX['scale_name'] = cur_name;
                        inner_tempX['label1'] = sample_type;
                        inner_tempX['label2'] = disease_state;
                        ...
                        if (graph.graph_type != "Scatter Plot") {
                            tempX['mapping'] = nested_values[p].values[0].values[0].values[0]['Multi_Mapping']; // get the mapping value pf probe p from the first array value, as the mapping value of probe is always unique
                ...
                tempX_outer['label2'] = disease_state;
          ...
          if( option == 1) // returns gene and probe name STAT1 ILMN_1543256
            {
              var prefix;
                  ...
                  if(graph.graph_type == "Scatter Plot" || label_num == 3) {
                    prefix = "";
                  }
                  else {
